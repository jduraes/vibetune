...
if errorlevel 1 (
  ...
  echo Example: Run-SIMH-VibeTune.cmd SBC_simh_std
  exit /b 1
)
...
if not exist %ROMWBW%\Binary\%ROM_NAME%.rom (
  ...
  echo Then retry:
  echo   Run-SIMH-VibeTune.cmd SBC_simh_std
  exit /b 1
)
...
echo Launching SIMH with ROM: %ROM_NAME%.rom
echo Using stock Sim.cfg disk mappings.
echo For ZPM3 testing, boot slice 4 via: 3.4
altairz80.exe Sim.cfg ..\..\Binary\%ROM_NAME%.rom
